...
export default function Support({ onOpen }: { onOpen?: (key: "journaling" | "community" | "nearby" | "emergency") => void }) {
  ...
  const CARDS: { title: string; desc: string; img: string; overlay: number; key: CardKey }[] = [
    { title: t.support.journaling, desc: t.support.journalingDesc, img: imgJournaling, overlay: 0.45, key: "journaling" },
    { title: t.support.community, desc: t.support.communityDesc, img: imgCommunity, overlay: 0.46, key: "community" },
    { title: t.support.nearby, desc: t.support.nearbyDesc, img: imgNearby, overlay: 0.45, key: "nearby" },
    { title: t.support.emergency, desc: t.support.emergencyDesc, img: imgEmergency, overlay: 0.35, key: "emergency" },
    { title: t.support.learn, desc: t.support.learnDesc, img: imgLearn, overlay: 0.45, key: null },
    { title: t.support.grow, desc: t.support.growDesc, img: imgGrow, overlay: 0.5, key: null },
  ];
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-3 gap-3 sm:gap-4">
        {CARDS.map((c) => (
          <button
            ...
            onClick={() => {
              if (!c.key) return;
              push({ source: "support", title: `${t.support.opened} ${c.title}`, body: c.desc });
              onOpen?.(c.key);
            }}
